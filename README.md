...
## Prompt-to-motion conversation demo

We provide a lightweight conversation loop that turns a free-form prompt into a spoken script and a queue of motion clips rendered on a VRM avatar. The backend orchestrates OpenAI planning and sequential motion generation, while the `three-vrm` web client plays the exported motion JSON files.

### Configure environment variables

Before launching the service, set the following variables in the shell where you will run the backend:

* `OPENAI_API_KEY` – personal API key for the OpenAI API used by `server/app.py`.
* `MDM_MODEL_PATH` – absolute path to the MDM checkpoint (for example `./save/humanml_trans_dec_512_bert-50steps/model000150000.pt`).

### Start the FastAPI backend

1. Activate the conda environment (`conda activate mdm`).
2. From the repository root, launch the server:
  ```shell
  uvicorn server.app:app --reload --port 8000
  ```
  Each request creates a session under `motions/<session_id>/segment_XX.json`, generated by `sample/generate.py` with the `--vrm_motion_path` override.

### Start the VRM web client

1. Place your avatar at `three-vrm/public/modelA.vrm` (any VRM 1.0 model works).
2. Install dependencies and start the dev server:
  ...
  npm start
  ```
  The Create React App development server proxies API calls to `http://localhost:8000` via the `package.json` proxy setting.

### Driving the demo

Open `http://localhost:3000`, type a conversation prompt (for example, “greet the audience, wave hello, then point to the horizon”), and submit. The UI shows the generated narration text and the movement plan. As each segment becomes available, it is fetched from the backend and played in sequence on the VRM. Facial expression sliders are available while playback is idle. You can inspect or reuse the motion JSON clips saved in the `motions/` directory.
